...
public class ApiPermissionHelper {
     ...
     */
    public boolean isCallerAllowed() throws WrongPackageCertificateException {
    ...
    private boolean isUidAllowed(int uid)
        ...
        App.log.warning("Uid is NOT allowed!");
        return false;
    }
    ...
    public boolean isPackageAllowed(String packageName) throws WrongPackageCertificateException {
        byte[] storedPackageCert = getCertificate(mContext, packageName);

        boolean isKnownPackage = storedPackageCert != null;
        if (!isKnownPackage) {
            App.log.warning("Package is NOT allowed! packageName: " + packageName);
            return false;
        }
        App.log.info("Package is allowed! packageName: " + packageName);
    ...
    public static void addCertificate(Context context, String packageName) {
        SharedPreferences sharedPref = context.getSharedPreferences(FILE_API,
                Context.MODE_PRIVATE);
        try {
            sharedPref.edit().putString(packageName,
                    Base64.encodeToString(getPackageCertificate(context, packageName), Base64.DEFAULT)).apply();
            App.log.info("Added permission for package:" + packageName);
        } catch (NameNotFoundException aE) {
            aE.printStackTrace();
        }
    }

    private static byte[] getCertificate(Context context, String packageName) {
        SharedPreferences sharedPref = context.getSharedPreferences(FILE_API,
                Context.MODE_PRIVATE);
        String cert = sharedPref.getString(packageName, null);
